...
function! s:highlight_from_to(line, start, end)
    ...
    while i != a:end
        ...
        if char == "\<space>" || !has_key(accepted_chars, char) || empty(char)
            if !is_first_word
                " If there is a primary highlight, add it to the last word.
                if to_hi_primary > 0
                    let hi_primary = printf("%s|%%%sc", hi_primary, to_hi_primary)
                elseif to_hi_secondary > 0
            ...
            if !is_first_word
                ...
                " If the is search forward, we want to be greedy; otherwise, we
